...
export const home = async (req, res) => {
    try {
        ...
        for(const student of students){
            const studentsTime = await StudentTime.findOne({ studentId: student._id }) // Use findOne instead of find

            const getStatus = (studentsTime) => {
                if (studentsTime && Array.isArray(studentsTime.timeIn) && Array.isArray(studentsTime.timeOut)) {
                    if(studentsTime.timeIn.length > 0 && studentsTime.timeOut.length > 0){
                        const lastTimeIn = new Date(studentsTime.timeIn[studentsTime.timeIn.length -1])
                        const lastTimeOut = new Date(studentsTime.timeOut[studentsTime.timeOut.length -1])
            ...
            studentsData.push({...student._doc, studentsTime, status: getStatus(studentsTime)})
        }
...
export const login = async (req, res) => {
    try {
        ...
        const studentName = await Student.findOne({fullName: fullName})
        ...
        return res.status(400).json({error: `Error in Log In Controller ${error}`})
    }
}
...
export const logout = async (req, res) => {
    try{
        ...
        return res.status(400).json({error: `Error in Log Out Controller ${error}`})
    }
}
...
export const io = async (req, res) => {
    try {
        ...
        if (studentTime && Array.isArray(studentTime.timeIn) && Array.isArray(studentTime.timeOut)) {
            if(studentTime.timeIn.length > 0 && studentTime.timeOut.length > 0){
                const lastTimeIn = new Date(studentTime.timeIn[studentTime.timeIn.length -1])
                const lastTimeOut = new Date(studentTime.timeOut[studentTime.timeOut.length -1])
        ...
        return res.status(400).json({error: `Error in Time In Out Controller ${error}`})
    }
}
...
export const ioOp = async (req, res) => {
    try {
        ...
        return res.status(400).json({error: `Error in In Out Operation ${error}`})
    }
}

export const signup = async (req, res) => {
   try {
    const {fullName, password, sport, sex, level} = req.body
    ...
    if(student) { return res.status(400).json({error: `${fullName} Already Exist`})}

    const newStudent = new Student({ ipAddress: ip, fullName: fullName, password: password, sport: sport, sex: sex, level:  level})
    ...
    return res.status(400).json({error: `Error in Sign Up Controller ${error}`})
   }
}
...
export const view = async (req, res) => {
    try {
        ...
        if(!studentTime) {
            return res.status(400).json({error: `No Time Table`})
        }
        ...
        const studentDatas = {...student._doc, studentTimeIn: formattedTimesIn, studentTimeOut: formattedTimesOut}
...
export const userEdit = async (req, res) => {
    try {
        ...
        return res.status(400).json({error: `Error in User Edit Controller ${error}`});
    }
}
...
export const userView = async (req, res) => {
    try {
        ...
        return res.status(200).json({studentData: studentDatas});
    } catch (error) {
        return res.status(400).json({error: `Error in User Edit Controller ${error}`});
    }
}